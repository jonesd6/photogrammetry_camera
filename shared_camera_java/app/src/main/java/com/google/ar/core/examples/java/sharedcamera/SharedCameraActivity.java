...
public class SharedCameraActivity extends AppCompatActivity
  ...
  DepthFrameAvailableListener depthImageAvailableListener = new DepthFrameAvailableListener();
  // Give a unique ID to depth/pose/image data from same frame.
  private int fileCounter = 0;
  NumberFormat formatter = new DecimalFormat("000");

  // Set height and width for the image saved from rendered surface.
  ...
  private int[] rawMask = new int[size];
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    Button captureDataButton = findViewById(R.id.capture_data);

    captureDataButton.setOnClickListener(new View.OnClickListener() {
  ...
  public void onImageAvailable(ImageReader imageReader) {
    ...
    if (image != null && image.getFormat()==ImageFormat.DEPTH16) {
      ...
      Log.d("DEBUG", "returned depth16 frame.");
    }
    if (image == null) {
  ...
  public void onDrawFrameARCore() throws CameraNotAvailableException {
    ...
    if (captureData) {
      captureData = false;
      Log.d("DEBUG", "capture data was true, now false");

      // pose, metadata, intrinsics
      saveData(camera.getPose(), camera.getImageIntrinsics());
      Log.d("DEBUG", "save data was called correctly.");
  ...
  public String xmpMetadata(Pose cameraPose, CameraIntrinsics cameraIntrinsics) {
    Log.d("DEBUG", "Creating XMP metadata.");
    String principalPointU = String.valueOf(cameraIntrinsics.getPrincipalPoint()[0]);
    String principalPointV = String.valueOf(cameraIntrinsics.getPrincipalPoint()[1]);

    StringBuilder builder = new StringBuilder();

    for (float val:cameraPose.getXAxis()) {
      builder.append(val);
      builder.append(" ");
    }
    for (float val:cameraPose.getYAxis()) {
      builder.append(val);
      builder.append(" ");

    }

    for (float val:cameraPose.getZAxis()) {
      builder.append(val);
      builder.append(" ");
    }
    ...
    for (float val:cameraPose.getTranslation()) {
      builder.append(val);
      builder.append(" ");
    }
    ...
    String distortion = "0 0 0 0 0";

    String metadataText = "<x:xmpmeta xmlns:x=\"adobe:ns:meta/\">" +
            ...
            "xcr:FocalLength35mm=\"28.4544382202237\" +\"" +
            "xcr:Skew=\"0\"" +
            "xcr:AspectRatio=\"1\"" +
            "xcr:PrincipalPointU=\"" + principalPointU + "\"" +
            "xcr:PrincipalPointV=\"" + principalPointV + "\"" +
  ...
  public void saveData(Pose cameraPose, CameraIntrinsics cameraIntrinsics) {


    String xmpMetadataText = xmpMetadata(cameraPose, cameraIntrinsics);

    String xmpFileName = formatter.format(fileCounter) + "PLH_IMG.xmp";
    String imageFileName = formatter.format(fileCounter) + "PLH_IMG.png";
    String depthFileName = formatter.format(fileCounter) + "PLH_DEPTH.txt";

    File xmpFile = new File(getExternalFilesDir(null), xmpFileName);

    FileOutputStream fos = null;
    try {
      fos = new FileOutputStream(xmpFile);
    } catch (FileNotFoundException e) {
      e.printStackTrace();
    }
    try {
      fos.write(xmpMetadataText.getBytes());
      fos.close();
    } catch (IOException e) {
      e.printStackTrace();
    }
    ...
    int pixelData[] = new int[imageHeight * imageWidth];
    ...
    for (int i = 0; i < imageHeight; i++) {
      for (int j = 0; j < imageWidth; j++) {
        int p = pixelData[i * imageWidth + j];
        int b = (p & 0x00ff0000) >> 16;
        int r = (p & 0x000000ff) << 16;
        int ga = p & 0xff00ff00;
        bitmapData[(imageHeight - i -1) * imageWidth + j] = ga | r | b;
      }
    }
    fos = null;
    Bitmap bmp = Bitmap.createBitmap(bitmapData, imageWidth, imageHeight, Bitmap.Config.ARGB_8888);
...
/*
    fos = null;
    File depthFile = new File(getExternalFilesDir(null), depthFileName);
    try  {
      fos = new FileOutputStream(depthFile);
      for (int i = 0; i < rawMask.length; i++) {
        fos.write(formatter.format(rawMask[i]).getBytes());
      }
      fos.close();
    } catch (FileNotFoundException e) {
      e.printStackTrace();
...
/*
    for (int y = 0; y < depthHeight; y++) {
      for (int x = 0; x < depthWidth; x++) {
        ...
        int newValue = extractRange(depthSample);
  ...
  private int extractRange(short sample) {
